...
def materialize(target_type: str, source: str, allowed_root: str | None, portable_clone: bool = False) -> tuple[Path, tempfile.TemporaryDirectory | None]:
    ...
    try:
        if portable_clone:
            ...
            porcelain.clone(source, str(destination), depth=1)
        else:
            subprocess.run([
        ...
        if isinstance(error, subprocess.CalledProcessError):
            ...
            detail = str(error) or "Repository clone failed"
        raise ValueError(f"Repository clone failed: {detail}") from error
    return destination, temp


def analyze_target(target_type: str, source: str, allowed_root: str | None = None, portable_clone: bool = False) -> dict:
    root, temp = materialize(target_type, source, allowed_root, portable_clone)
    try:
...
def main() -> None:
    ...
    print(json.dumps(analyze_target(args.target_type, args.source, args.allowed_root)))
