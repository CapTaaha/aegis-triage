...
const ArchitectureGuide = ({ graph, site, correlations }: ArchitectureGuideProps) => {
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1fr_.72fr]">
      <Card className="rounded-3xl border-slate-700 bg-slate-900/80 text-slate-100"><CardHeader><CardTitle className="flex items-center gap-2 text-cyan-300"><Workflow className="h-5 w-5" /> Analysis architecture</CardTitle></CardHeader><CardContent><ol className="grid gap-3 text-sm text-slate-400 sm:grid-cols-2">{['Authorized target and allowlist validation', 'tree-sitter AST parsing with fallback coverage', 'Source, sink, sanitizer, and taint analysis', 'Same-origin passive page and form inventory', 'Route-to-page evidence correlation', 'Human verification and full report export'].map((step, index) => <li key={step} className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4"><span className="mb-2 block font-mono text-xs text-emerald-400">0{index + 1}</span>{step}</li>)}</ol></CardContent></Card>
      <Card className="rounded-3xl border-slate-700 bg-slate-900/80 text-slate-100"><CardHeader><CardTitle className="flex items-center gap-2 text-emerald-300"><Download className="h-5 w-5" /> Export current run</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm text-slate-400">Exports include source findings, call edges, passive crawl evidence, route correlations, and triage recommendations.</p><Button onClick={() => download('aegis-triage-report.json', 'application/json', JSON.stringify({ graph, site, correlations }, null, 2))} className="w-full rounded-xl bg-emerald-500 text-slate-950 hover:bg-emerald-400"><FileJson className="mr-2 h-4 w-4" /> Export JSON</Button><Button onClick={exportMarkdown} variant="outline" className="w-full rounded-xl border-cyan-400/30 text-cyan-200 hover:bg-cyan-400/10">Export Markdown</Button></CardContent></Card>
    </div>
  );
};
